...
const ExerciseTable = ({ exercise }) => {
    ...
    const addNewSet = () => {
        ...
        useRoutineStore.getState().addSetToExercise(exercise._id, newSet);
    };

    const handleDeleteSet = (index) => {
        index === 0 ?
            showInfoAlert("You can't delete the first set") :
            useRoutineStore.getState().deleteSetFromExercise(exercise._id, index);
    };
            ...
            {sets.map((set, index) => (
                    <View key={index} style={styles.table_row}>
                    {
                        ...
                        <TextInput
                            ...
                            maxLength={3}
                            onChangeText={(value) => useRoutineStore.getState().updateSet(exercise, index, 'kilograms', value)}
                        ...
                        <TextInput
                            ...
                            keyboardType='numeric'
                            maxLength={3}
                            onChangeText={(value) => useRoutineStore.getState().updateSet(exercise, index, 'reps', value)}
